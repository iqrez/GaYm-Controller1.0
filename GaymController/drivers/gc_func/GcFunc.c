...
NTSTATUS GcFuncEvtDeviceAdd(WDFDRIVER Driver, PWDFDEVICE_INIT DeviceInit);
VOID GcFuncEvtIoDeviceControl(WDFQUEUE Queue, WDFREQUEST Request,
                               size_t OutputBufferLength, size_t InputBufferLength,
                               ULONG IoControlCode);

static VOID GcOnVhfReadyForWrite(PVOID Context);
static VOID GcOnVhfProcessOutput(PVOID Context, PHID_XFER_PACKET OutputPacket);
static VOID GcSendInputReport(PFUNC_CONTEXT ctx, PVPAD_STATE state);
...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING RegistryPath)
{
    ...
    WDF_DRIVER_CONFIG_INIT(&config, GcFuncEvtDeviceAdd);
...
NTSTATUS GcFuncEvtDeviceAdd(WDFDRIVER Driver, PWDFDEVICE_INIT DeviceInit)
{
    ...
    PFUNC_CONTEXT ctx = GcFuncGetContext(device);
    ...
    qcfg.EvtIoDeviceControl = GcFuncEvtIoDeviceControl;
    status = WdfIoQueueCreate(device, &qcfg, WDF_NO_OBJECT_ATTRIBUTES, &ctx->IoctlQueue);
    if (!NT_SUCCESS(status)) return status;

    VHF_CONFIG cfg;
    VHF_CONFIG_INIT(&cfg, WdfDeviceWdmGetDeviceObject(device), g_GcReportDescriptor, sizeof(g_GcReportDescriptor));
    cfg.EvtVhfReadyForWrite = GcOnVhfReadyForWrite;
    cfg.EvtVhfProcessOutputReport = GcOnVhfProcessOutput;
