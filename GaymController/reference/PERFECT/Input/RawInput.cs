...
namespace WootMouseRemap
{
    public sealed class RawInput : IDisposable
    {
        ...
        private unsafe void ProcessRawInput(IntPtr hRawInput)
        {
            ...
            fixed (byte* p = buffer)
            {
                ...
                else if (header.dwType == RIM_TYPEMOUSE)
                {
                    ...
                    if ((mouse.usButtonFlags & RI_MOUSE_BUTTON_1_DOWN) != 0) MouseButton?.Invoke(MouseInput.Left, true);
                    if ((mouse.usButtonFlags & RI_MOUSE_BUTTON_1_UP) != 0) MouseButton?.Invoke(MouseInput.Left, false);
                    if ((mouse.usButtonFlags & RI_MOUSE_BUTTON_2_DOWN) != 0) MouseButton?.Invoke(MouseInput.Right, true);
                    if ((mouse.usButtonFlags & RI_MOUSE_BUTTON_2_UP) != 0) MouseButton?.Invoke(MouseInput.Right, false);
                    if ((mouse.usButtonFlags & RI_MOUSE_BUTTON_3_DOWN) != 0) MouseButton?.Invoke(MouseInput.Middle, true);
                    if ((mouse.usButtonFlags & RI_MOUSE_BUTTON_3_UP) != 0) MouseButton?.Invoke(MouseInput.Middle, false);
                    if ((mouse.usButtonFlags & RI_MOUSE_BUTTON_4_DOWN) != 0) MouseButton?.Invoke(MouseInput.XButton1, true);
                    if ((mouse.usButtonFlags & RI_MOUSE_BUTTON_4_UP) != 0) MouseButton?.Invoke(MouseInput.XButton1, false);
                    if ((mouse.usButtonFlags & RI_MOUSE_BUTTON_5_DOWN) != 0) MouseButton?.Invoke(MouseInput.XButton2, true);
                    if ((mouse.usButtonFlags & RI_MOUSE_BUTTON_5_UP) != 0) MouseButton?.Invoke(MouseInput.XButton2, false);
        ...
        public void Dispose() => _msgWin.RawMessage -= OnWndMessage;
        ...
        [StructLayout(LayoutKind.Sequential)]
        private struct RAWINPUTHEADER { public int dwType, dwSize; public IntPtr hDevice, wParam; }
        [StructLayout(LayoutKind.Sequential)]
